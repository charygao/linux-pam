	    ...
	    const char **user_envfile, int *user_readenv)
{
    ...
    *conffile = DEFAULT_CONF_FILE;
	...
	if (!strcmp(*argv,"debug"))
	    ctrl |= PAM_DEBUG_ARG;
	else if ((str = pam_str_skip_prefix(*argv, "conffile="))) {
	  if (str[0] == '\0') {
	...
	} else if ((str = pam_str_skip_prefix(*argv, "envfile="))) {
	  if (str[0] == '\0') {
	    pam_syslog (pamh, LOG_ERR,
	...
	} else if ((str = pam_str_skip_prefix(*argv, "user_envfile="))) {
	  if (str[0] == '\0') {
	    pam_syslog (pamh, LOG_ERR,
	...
	} else if ((str = pam_str_skip_prefix(*argv, "readenv="))) {
	  *readenv = atoi(str);
	} else if ((str = pam_str_skip_prefix(*argv, "user_readenv="))) {
...
_parse_line (const pam_handle_t *pamh, const char *buffer, VAR *var)
{
  ...
  while ((length = strspn(ptr, " \t")) > 0) {
    ...
    D((ptr));
    if ((tmpptr = pam_str_skip_prefix(ptr, "DEFAULT="))) {
      ptr = tmpptr;
      D(("Default arg found: <%s>", ptr));
      valptr=&(var->defval);
    } else if ((tmpptr = pam_str_skip_prefix(ptr, "OVERRIDE="))) {
      ptr = tmpptr;
